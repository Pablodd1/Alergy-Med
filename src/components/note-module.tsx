...
export function NoteModule({ visitId, onBack }: NoteModuleProps) {
  ...
  const downloadPDF = () => {
    try {
      ...
      // Footer
      yPosition += 10
      addText(`\nGenerated by Roman Super Allergist Assistant on ${new Date().toLocaleDateString()}`, 10, false)
  ...
  const downloadDOCX = () => {
    try {
      ...
      sections.push(
        new Paragraph({
          text: `\nGenerated by Roman Super Allergist Assistant on ${new Date().toLocaleDateString()}`,
          spacing: { before: 200 }
        })
      )
      sections.push(
            ...
            <div className="space-y-6">
              ...
              {extraction?.needsConfirmation && extraction.needsConfirmation.length > 0 && (
                <Card className="border-yellow-200 bg-yellow-50">
                  <CardHeader>
                    <CardTitle className="text-yellow-800">⚠️ Needs Confirmation / Missing Data</CardTitle>
                  </CardHeader>
                  <CardContent>
              ...
              {extraction?.sourceQualityFlags && extraction.sourceQualityFlags.length > 0 && (
                <Card className="border-orange-200 bg-orange-50">
                  <CardHeader>
                    <CardTitle className="text-orange-800">📊 Source Quality Flags</CardTitle>
                  </CardHeader>
                  <CardContent>
